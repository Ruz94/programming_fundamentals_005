...
/*
  This function allows you to find a value from an object which fulfils a criteria. It receives an object and a matcher
  function. It should test each value in the object against the matcher function and if a matching value is found, the 
  value should be returned. Otherwise, return null.
*/
function find(obj, matcherFunc) {
  const values = Object.values(obj);
  for (let elements of values) {
    if (matcherFunc(elements)) {
      return elements;
    }
  }
  return null;
...
function every(obj, func) {
  ...
  for (let elements of values) {
    return !func(elements) ? (resultMatch = false) : resultMatch;
  }
}
/*
...
function some(obj, func) {
  ...
  for (let elements of values) {
    return func(elements) ? (resultMatch = true) : resultMatch;
  }
}
